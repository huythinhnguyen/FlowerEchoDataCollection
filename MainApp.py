...
import Library.Device as Device
import Library.Logger as Logger

import Library.Utils as Library
import Library.Settings as Settings
import Library.Ports as Ports
import Library.Sonar as Sonar
import Library.TurnTable as TurnTable
...
LIBPATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Library')
...
class HelloApp:
    def __init__(self, master):        
        ...
        self.connect_rotation_table = Settings.connect_rotation_table
        ...
        if self.connect_rotation_table:
            self.logger.print_log('Connecting to rotation table')
            self.rotation_table = TurnTable.TurnTable(offset=Settings.rotation_table_offset, verbose=True)
            self.rotation_table_angles = Settings.rotation_table_angles
    ...
    def do_measurement(self, event):
        ...
        shutil.copy(os.path.join(LIBPATH,'Settings.py'), data_folder + '/Settings.py')
        ...
        n_table_angles = len(self.rotation_table_angles)
        all_data = numpy.empty((7000, 2, repeats, n_positions, n_table_angles))
        for table_angle_i, table_angle in enumerate(self.rotation_table_angles):
            self.rotation_table.position(table_angle, block=False)
            time.sleep(Settings.turntable_pause)
            for position_i, position in enumerate(self.servo_positions):
                if self.connect_servo:
                ...
                for repetition in range(repeats):
                    ...
                    all_data[:, :, repetition, position_i, table_angle_i] = data
                    time.sleep(Settings.measurement_pause)

                current_measurement_data = all_data[:, :, :, position_i, table_angle_i]
